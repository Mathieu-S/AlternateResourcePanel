...
namespace KSPAlternateResourcePanel
{
    ...
    internal class ARPWindowResourceConfig : MonoBehaviourWindowPlus
    {
        private DropDownList ddlDisplayValueAs;

        private DropDownList ddlMonType;

        //are we currently dragging a resource
        internal bool DraggingResource;

        internal bool DropWillReorderList;

        /// <summary>
        ///     Icon the Mouse is Over
        /// </summary>
        internal ResourcePosition iconOver;
        ...
        /// </summary>
        internal ResourcePosition resourceDrag;

        internal int resourceDragIndex = -1;
        internal int resourceInsertIndex = -1;

        /// <summary>
        ///     Resource the mouse is over
        /// </summary>
        internal ResourcePosition resourceOver;

        /// <summary>
        ///     Is the mouse over the top half of the resource - ie drop it above or below the resource
        /// </summary>
        internal bool resourceOverUpper;

        private int ResourceToShowAlarm;
        private bool ResourceToShowAlarmChangeNeeded;
        private int ResourceToShowAlarmChanger;
        private readonly int ScrollAreaWidth = 440;
        ...
        private readonly int WindowHeight = 480;
        ...
        internal override void OnAwake()
        {
            ...
            ddlMonType = new DropDownList(EnumExtensions.ToEnumDescriptions<ResourceSettings.MonitorDirections>(),
                this);
            ddlMonType.SetListBoxOffset(vectMonTypeOffset - ScrollPosition);
            ddlMonType.OnSelectionChanged += ddlMonType_OnSelectionChanged;

            ddlDisplayValueAs = new DropDownList(EnumExtensions.ToEnumDescriptions<ResourceSettings.DisplayUnitsEnum>(),
                this);
        ...
        private void ddlMonType_OnSelectionChanged(DropDownList sender, int OldIndex, int NewIndex)
        {
            settings.Resources[ResourceToShowAlarm].MonitorDirection = (ResourceSettings.MonitorDirections) NewIndex;
        }

        private void ddlDisplayValueAs_OnSelectionChanged(DropDownList sender, int OldIndex, int NewIndex)
        {
        ...
        internal override void DrawWindow(int id)
        {
            ...
            ScrollPosition =
                GUILayout.BeginScrollView(ScrollPosition, new GUIStyle(), GUILayout.Width(ScrollAreaWidth));
            ...
            {
                var item = settings.Resources.Values.ElementAt(i);
                ...
                if (!item.IsSeparator)
                {
                    ...
                    if (Event.current.type == EventType.Repaint)
                    {
                        ...
                        lstResPositions.Add(new ResourcePosition(item.id, item.name, IconRect, ScrollAreaWidth - 20,
                            ResourceToShowAlarm == item.id));
                    }
                    ...
                    if (GUILayout.Button(item.name, SkinsLibrary.CurrentSkin.label, GUILayout.Width(NameWidth)))
                    {
                        ...
                        else
                        {
                            ...
                            ddlMonType.SelectedIndex =
                                (int) settings.Resources[ResourceToShowAlarmChanger].MonitorDirection;
                            ddlDisplayValueAs.SelectedIndex =
                                (int) settings.Resources[ResourceToShowAlarmChanger].DisplayValueAs;
                        }
                ...
                else
                {
                    ...
                    if (Event.current.type == EventType.Repaint)
                        if (lstResPositions.Count > 0)
                        {
                            ...
                            lstResPositions.Add(new ResourcePosition(item.id, "Separator", IconRect,
                                ScrollAreaWidth - 20, ResourceToShowAlarm == item.id));
                        }
                }
                ...
                if (i > 0)
                {
                    if (GUILayout.Button(new GUIContent("↑", "Move Up"), GUILayout.Width(21))) SwapResource(i - 1, i);
                }
                else
                {
                ...
                {
                    if (GUILayout.Button("↓", GUILayout.Width(21))) SwapResource(i, i + 1);
                }
                else
                {
                    GUILayout.Space(21 + 4);
                ...
                if (i > 0)
                {
                    if (GUILayout.Button(new GUIContent("↑↑", "Move to Top"), GUILayout.Width(27))) MoveResource(i, 0);
                }
                else
                {
                ...
                if (!item.IsSeparator)
                {
                    ...
                    if (GUILayout.Button(
                        string.Format("{0}/{1}", settings.Resources[item.id].MonitorWarningLevel,
                            settings.Resources[item.id].MonitorAlertLevel), GUILayout.Width(58)))
                    {
                        if (ResourceToShowAlarmChanger == item.id)
                        {
                            ResourceToShowAlarmChanger = 0;
                ...
                {
                    if (GUILayout.Button("Delete", GUILayout.Width(75 + 58 + 4))) SepToDelete = item.id;
                }
                ...
                if (ResourceToShowAlarm == item.id)
                {
                    ...
                    settings.Resources[item.id].MonitorWarningLevel = (int) Math.Round(
                        GUILayout.HorizontalSlider(settings.Resources[item.id].MonitorWarningLevel, 0, 100,
                            GUILayout.Width(220)));
                    GUILayout.Label(settings.Resources[item.id].MonitorWarningLevel + "%", GUILayout.Width(35));
                    GUILayout.EndHorizontal();

                    GUILayout.BeginHorizontal();
                    GUILayout.Label("Alert Level:", GUILayout.Width(90));
                    settings.Resources[item.id].MonitorAlertLevel = (int) Math.Round(
                        GUILayout.HorizontalSlider(settings.Resources[item.id].MonitorAlertLevel, 0, 100,
                            GUILayout.Width(220)));
                    GUILayout.Label(settings.Resources[item.id].MonitorAlertLevel + "%", GUILayout.Width(35));
                    GUILayout.EndHorizontal();
                    GUILayout.EndVertical();

                    if (settings.Resources[item.id].MonitorDirection == ResourceSettings.MonitorDirections.Low)
                        settings.Resources[item.id].MonitorWarningLevel = Mathf.Clamp(
                            settings.Resources[item.id].MonitorWarningLevel,
                    ...
                    if (settings.Resources[item.id].SplitLastStage && (
                            PartResourceLibrary.Instance.resourceDefinitions[item.id].resourceFlowMode ==
                    ...
                    )
                    {
                        ...
                        DrawToggle(ref settings.Resources[item.id].ShowReserveLevels,
                            new GUIContent("Show Reserve Levels", "instead of Whole Vessel/Last Stage split"),
                            Styles.styleToggle);
                        GUILayout.EndHorizontal();
                    }
            ...
            if (SepToDelete != 0)
            {
                ...
                var lstTemp = settings.Resources.Values.ToList();
                settings.Resources = lstTemp.ToDictionary(x => x.id);
            }

            GUILayout.BeginHorizontal();
            if (DrawButton("Add Separator")) AddSeparatorAtEnd();
            if (DrawButton("Sort Groups")) SortGroups();

            if (DrawButton("Save")) settings.Save();
            GUILayout.EndHorizontal();
            ...
            if (DraggingResource && DropWillReorderList && resourceOver != null)
            {
                ...
                if (resourceInsertIndex < lstResPositions.Count)
                    ...
                    rectResMoveY = lstResPositions[lstResPositions.Count - 1].resourceRect.y +
                                   lstResPositions[lstResPositions.Count - 1].resourceRect.height;
        ...
        private void IconMouseEvents()
        {
            ...
            if (MousePosition.y > 54 && MousePosition.y < WindowHeight - 26)
            {
                ...
                resourceOver = lstResPositions.FirstOrDefault(x =>
                    x.resourceRect.Contains(MousePosition + ScrollPosition - new Vector2(8, 54)));
                if (resourceOver != null)
                {
                    resourceOverUpper =
                        (MousePosition + ScrollPosition - new Vector2(8, 54)).y - resourceOver.resourceRect.y <
                        resourceOver.resourceRect.height / 2;
                ...
                iconOver = lstResPositions.FirstOrDefault(x =>
                    x.iconRect.Contains(MousePosition + ScrollPosition + -new Vector2(8, 54)));

                //Will the drop actually change the list
                DropWillReorderList = resourceInsertIndex != resourceDragIndex &&
                                      resourceInsertIndex != resourceDragIndex + 1;
            }
            else
            {
                resourceOver = null;
        ...
        internal override void OnGUIEvery()
        {
            ...
            if (DraggingResource && resourceDrag != null)
            {
                ...
                if (resourceDrag.name != "Separator")
                {
                    ...
                    ;
                    Rect ResPos = new Rect(Input.mousePosition.x + 55, Screen.height - Input.mousePosition.y - 6, 32,
                        16);
        ...
        {
            var lstToSort = new List<ResourceSettings>();
            var lstFinal = new List<ResourceSettings>();
            for (var i = 0; i < settings.Resources.Count; i++)
            {
                if (i == 0 || settings.Resources.ElementAt(i).Value.IsSeparator)
                {
                    //Sort the current list
                    if (lstToSort.Count > 0) lstFinal.AddRange(lstToSort.OrderBy(x => x.name));
                    //reset the sort list
        ...
        private void SwapResource(int indexFirst, int indexSecond)
        {
            var lstTemp = settings.Resources.Values.ToList();
            var tempItem = lstTemp[indexFirst];
            lstTemp[indexFirst] = lstTemp[indexSecond];
            lstTemp[indexSecond] = tempItem;
            settings.Resources = lstTemp.ToDictionary(x => x.id);
        }

        private void MoveResource(int indexFrom, int InsertIndex)
        {
            ...
            if (InsertIndex > indexFrom)
                for (var i = indexFrom; i < InsertIndex - 1; i++)
                {
                    LogFormatted_DebugOnly("Swap Resource - {0}<->{1}", i, i + 1);
                    SwapResource(i, i + 1);
                }
            else
                for (var i = indexFrom; i > InsertIndex; i--)
                    SwapResource(i, i - 1);
        }
        ...
        internal class ResourcePosition
        {
            ...
            private readonly int _resWidth;

            internal ResourcePosition(int id, string name, Rect iconRect, int width, bool Expanded)
            {
                this.id = id;
                this.name = name;
