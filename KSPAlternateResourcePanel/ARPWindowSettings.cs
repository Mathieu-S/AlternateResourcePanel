...
{
    internal class ARPWindowSettings : MonoBehaviourWindowPlus
    {
        ...
        private int intBlizzyToolbarMissingHeight;

        private int intStylingWindowHeightAdjustments;
        internal KSPAlternateResourcePanel mbARP;
        private readonly int MinWindowHeight = 136;
        internal Settings settings;
        private int SettingsAreaWidth = 314; //284;
        ...
        internal override void OnAwake()
        {
            ...
            ddlSettingsSkin = new DropDownList(EnumExtensions.ToEnumDescriptions<Settings.DisplaySkin>(),
                (int) settings.SelectedSkin, this);
            ...
            ddlSettingsRateStyle = new DropDownList(EnumExtensions.ToEnumDescriptions<Settings.RateDisplayEnum>(),
                (int) settings.RateDisplayType, this);
            ddlSettingsRateStyle.OnSelectionChanged += ddlSettingsRateStyle_OnSelectionChanged;

            ddlSettingsButtonStyle = new DropDownList(EnumExtensions.ToEnumDescriptions<ButtonStyleEnum>(),
                (int) settings.ButtonStyleChosen, this);
        ...
        private void ddlSettingsAlarmsAlert_OnSelectionChanged(DropDownList sender, int OldIndex, int NewIndex)
        {
        ...
        private void ddlSettingsAlarmsWarning_OnSelectionChanged(DropDownList sender, int OldIndex, int NewIndex)
        {
        ...
        private void ddlSettingsSkin_SelectionChanged(DropDownList sender, int OldIndex, int NewIndex)
        {
        ...
        private void ddlSettingsRateStyle_OnSelectionChanged(DropDownList sender, int OldIndex, int NewIndex)
        {
            settings.RateDisplayType = (Settings.RateDisplayEnum) NewIndex;
            settings.Save();
        }

        private void ddlSettingsButtonStyle_OnSelectionChanged(DropDownList sender, int OldIndex, int NewIndex)
        {
        ...
        {
            var retDDl = new DropDownList(Names, this);

            if (Names.Contains(Selected)) retDDl.SelectedIndex = Array.FindIndex(Names, x => x == Selected);
            return retDDl;
        }
        ...
        private void DrawWindow_General()
        {
            ...
            //    settings.Save();
            if (DrawToggle(ref settings.ShowRatesForParts, "Show Rates for Parts", Styles.styleToggle)) settings.Save();

            GUILayout.BeginHorizontal();
            if (DrawToggle(ref settings.RatesUseUT, "UT", Styles.styleToggle, GUILayout.Width(60)))
                settings.Save();
            var NotUT = !settings.RatesUseUT;
            ...
            GUILayout.BeginVertical();
            GUILayout.Space(2);
            if (DrawToggle(ref settings.SplitLastStage,
                new GUIContent("Enabled", "Turn this off to show single green bars and no last stage separation."),
                Styles.styleToggle))
                settings.Save();
            if (settings.SplitLastStage)
            {
                GUILayout.BeginHorizontal();
                var NotRight = !settings.StageBarOnRight;
                ...
                if (DrawToggle(ref settings.StageBarOnRight, "On Right", Styles.styleToggle)) settings.Save();
                GUILayout.EndHorizontal();
            }
            ...
            if (DrawButton("Configure Resource Settings"))
            {
                ...
                if (mbARP.windowResourceConfig.Visible)
                {
                    ...
                    if (mbARP.windowMain.WindowRect.x + mbARP.windowMain.WindowRect.width / 2 > Screen.width / 2)
                        mbARP.windowResourceConfig.WindowRect.x =
                            mbARP.windowMain.WindowRect.x - mbARP.windowResourceConfig.WindowRect.width;
                    else
                        mbARP.windowResourceConfig.WindowRect.x =
                            mbARP.windowMain.WindowRect.x + mbARP.windowMain.WindowRect.width;
                }
            }

            if (DrawToggle(ref settings.ShowWindowOnResourceMonitor,
                new GUIContent("Show Window on Alarm",
                    "If an alarm is exceeded then popup the window if it is not already visible. Will hide again when acknowledged."),
                Styles.styleToggle))
                settings.Save();
            GUILayout.EndVertical();
            GUILayout.EndHorizontal();
        ...
        private void DrawWindow_Styling()
        {
            ...
            if (!settings.BlizzyToolbarIsAvailable)
                ...
                {
                    if (GUILayout.Button(
                        new GUIContent("Not Installed. Click for Toolbar Info",
                            "Click to open your browser and find out more about the Common Toolbar"),
                        Styles.styleTextCenterGreen))
            ...
            {
                if (DrawToggle(ref settings.AppLauncherMutuallyExclusive,
                    new GUIContent("Hide when other Apps show",
                        "Hide the ARP when other stock Apps display (like the stock Resource App)"),
                    Styles.styleToggle))
                {
            ...
            GUILayout.Label(
                new GUIContent("Iconsets:",
                    "Select the order of priority for choosing icons. Highest priority to the left"),
                Styles.styleStageTextHead);
            GUILayout.Space(2);
            GUILayout.Label(new GUIContent("Separator:", "Padding around resource separators"),
                Styles.styleStageTextHead);
            ...
            GUILayout.BeginVertical();
            GUILayout.BeginHorizontal();
            for (var i = 0; i < settings.lstIconOrder.Count; i++)
            {
                if (i > 0)
                    if (GUILayout.Button("<->", GUILayout.Width(30)))
                    {
                        var strTemp = settings.lstIconOrder[i];
                ...
                GUILayout.Label(Resources.IconOrderContent(settings.lstIconOrder[i]), Styles.styleTextCenter,
                    GUILayout.Width(40));
            }
            ...
            GUILayout.BeginHorizontal();
            GUILayout.Space(4);
            settings.SpacerPadding = (int) GUILayout.HorizontalSlider(settings.SpacerPadding, 0, 5,
                GUILayout.Width(168 + mbARP.windowMain.IconAlarmOffset));
            GUILayout.Space(3);
            ...
            if (DrawToggle(ref settings.HideEmptyResources, "Hide Empty Resources", Styles.styleToggle))
                settings.Save();
            if (DrawToggle(ref settings.HideFullResources, "Hide Full Resources", Styles.styleToggle)) settings.Save();
            GUILayout.BeginHorizontal();
            GUILayout.Label("After:");
            settings.HideAfter = (int) GUILayout.HorizontalSlider(settings.HideAfter, 0, 10,
                GUILayout.Width(128 + mbARP.windowMain.IconAlarmOffset));
            GUILayout.Space(3);
            ...
            {
                if (settings.ButtonStyleToDisplay == ButtonStyleEnum.Launcher ||
                    settings.ButtonStyleToDisplay == ButtonStyleEnum.StockReplace)
        ...
        private void DrawWindow_Alarms()
        {
            ...
            if (DrawToggle(ref settings.AlarmsEnabled, "Enable Alarms Functionality", Styles.styleToggle))
                settings.Save();
            if (DrawToggle(ref settings.AlarmsVolumeFromUI, "Use KSP UI Volume", Styles.styleToggle)) settings.Save();
            GUILayout.BeginHorizontal();
            GUILayout.Space(10);
            if (settings.AlarmsVolumeFromUI)
                GUILayout.HorizontalSlider(GameSettings.UI_VOLUME * 100, 0, 100, GUILayout.Width(160));
            else
                settings.AlarmsVolume =
                    GUILayout.HorizontalSlider(settings.AlarmsVolume * 100, 0, 100, GUILayout.Width(160)) / 100;
            GUILayout.Label(KSPAlternateResourcePanel.audioController.VolumePct + "%");
            GUILayout.EndHorizontal();
            GUILayout.EndVertical();
            GUILayout.EndHorizontal();
        ...
        private void DrawTestSoundButton(AudioClip clip, int Repeats)
        {
            var blnStop = false;
        ...
        private void DrawWindow_Staging()
        {
            //Staging
            GUILayout.BeginHorizontal(Styles.styleSettingsArea, GUILayout.Width(SettingsAreaWidth),
                GUILayout.Height(64)); //, GUILayout.Height(84));
            GUILayout.BeginVertical(GUILayout.Width(60));
            ...
            if (settings.StagingEnabled)
            {
                ...
                if (settings.StagingEnabledInMapView)
                    if (DrawToggle(ref settings.StagingEnabledSpaceInMapView, "Allow Space Bar in Mapview",
                        Styles.styleToggle))
                        settings.Save();
            ...
            if (settings.StagingEnabled)
            {
                ...
                if (settings.AutoStagingEnabled)
                {
                    var AutoStagingDelay = (float) settings.AutoStagingDelayInTenths / 10;
                    GUILayout.BeginHorizontal();
                    if (DrawHorizontalSlider(ref AutoStagingDelay, 0.1f, 3f,
                        GUILayout.Width(148 + mbARP.windowMain.IconAlarmOffset)))
                    {
                        settings.AutoStagingDelayInTenths = (int) (AutoStagingDelay * 10);
                        settings.Save();
                    }
        ...
        private void DrawWindow_About()
        {
            ...
            {
                var intDots = Convert.ToInt32(Math.Truncate(DateTime.Now.Millisecond / 250d)) + 1;
            ...
            else
            {
                if (settings.VersionAvailable)
                    GUILayout.Label(
                        string.Format("{0} @ {1}", settings.VersionWeb, settings.VersionCheckDate_SuccessString),
                        Styles.styleTextYellowBold);
                else
                    GUILayout.Label(
                        string.Format("{0} @ {1}", settings.VersionWeb, settings.VersionCheckDate_SuccessString),
                        Styles.styleTextGreen);
            }
            ...
            if (settings.VersionAvailable)
            {
                ...
                if (GUILayout.Button("Updated Version Available", Styles.styleTextYellowBold))
                    Application.OpenURL("https://github.com/TriggerAu/KSPAlternateResourcePanel/releases");
                GUILayout.EndHorizontal();
            }
            ...
            if (GUILayout.Button("Click Here", Styles.styleTextCenterGreen))
                Application.OpenURL("https://github.com/TriggerAu/KSPAlternateResourcePanel/");
            if (GUILayout.Button("Click Here", Styles.styleTextCenterGreen))
        ...
        internal void UpdateWindowRect()
        {
            ...
            var blnKSPStyle = SkinsLibrary.CurrentSkin.name == "Default";
            Rect rectWindowSettings = new Rect(rectWindowMain)
                {y = rectWindowMain.y + rectWindowMain.height - 2, height = WindowHeight};
            ...
            if (Screen.height < rectWindowSettings.y + rectWindowSettings.height)
            {
                if (0 < rectWindowSettings.x - rectWindowSettings.width)
                {
                    ...
                    rectWindowSettings.y =
                        Mathf.Clamp(rectWindowMain.y, 0, Screen.height - rectWindowSettings.height + 1);
                }
            ...
            WindowRect = rectWindowSettings.ClampToScreen(ClampToScreenOffset);
        }
        ...
        internal enum ButtonStyleEnum
        {
            ...
            [Description("Replace Stock Resources App")]
            StockReplace
        }
