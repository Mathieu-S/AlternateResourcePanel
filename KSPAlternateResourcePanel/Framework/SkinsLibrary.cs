...
namespace KSPPluginFramework
{
    internal static class SkinsLibrary
    {
        ...
        /// </summary>
        internal static GUISkin CurrentSkin => _CurrentSkin;

        /// <summary>
        ///     Will return the current Tooltip style.
        ///     The Tooltip style will be the custom style called "Tooltip" or in the KSP and Unity Default skins I have created a
        ///     tooltip style, in a custom skin it will default back to the label style from the skin
        /// </summary>
        internal static GUIStyle CurrentTooltip { get; private set; }
        ...
        internal static void SetCurrent(DefSkinType DefaultSkin)
        {
            ...
            switch (DefaultSkin)
            {
                case DefSkinType.Unity:
                    _CurrentSkin = DefUnitySkin;
                    break;
                case DefSkinType.KSP:
        ...
        private static void SetCurrentTooltip()
        {
            ...
            {
                CurrentTooltip = GetStyle(_CurrentSkin, "Tooltip");
            }
            else
            {
                //otherwise lets build a style for the defaults or take the label style otherwise
                if (_CurrentSkin == DefUnitySkin)
                    CurrentTooltip = new GUIStyle(DefUnitySkin.box);
                else if (_CurrentSkin == DefKSPSkin)
                    CurrentTooltip = GenDefKSPTooltip();
                else
                    CurrentTooltip = _CurrentSkin.label;
            }
        }
        ...
        {
            if (List.ContainsKey(SkinID)) return (GUISkin) MonoBehaviourExtended.Instantiate(List[SkinID]);
        ...
        internal static void AddSkin(string SkinID, GUISkin NewSkin, bool SetAsCurrent = false)
        {
            NewSkin.name = SkinID;
            if (List.ContainsKey(SkinID))
                List[SkinID] = NewSkin;
        ...
        internal static bool RemoveSkin(string SkinID)
        {
        ...
        internal static bool StyleExists(DefSkinType DefaultSkin, string StyleID)
        {
            if (DefaultSkin == DefSkinType.KSP)
                return StyleExists(DefKSPSkin, StyleID);
            if (DefaultSkin == DefSkinType.Unity)
        ...
        internal static void AddStyle(ref GUISkin SkinToAction, GUIStyle NewStyle)
        {
            ...
            {
                MonoBehaviourExtended.LogFormatted("No Name Provided in the Style to add to {0}. Cannot add this.",
                    SkinToAction.name);
                return;
            }
