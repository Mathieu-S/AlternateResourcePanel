...
{
    internal class ARPWindow : MonoBehaviourWindowPlus
    {
        internal int Icon2BarOffset_Left = 40;
        internal int Icon2BarOffset_Right = 40 + 140;

        //public Rect rectIcon;
        internal int IconAlarmOffset = 12;
        internal int intLineHeight = 20;
        ...
        private void ARPWindow_onWindowMoveComplete(MonoBehaviourWindow sender)
        {
        ...
        internal override void DrawWindow(int id)
        {
            ...
            {
                LogFormatted("DrawWindow: GUILayout not ready yet");
                return;
            }
            ...
            else
            {
                int ResourceID;
                Rect rectBar;
                for (var i = 0; i < mbARP.lstResourcesToDisplay.Count; i++)
                {
                    ...
                    if (ResourceID == 0)
                    {
                        ...
                        GUILayout.Label("", Styles.styleSeparatorH, GUILayout.Width(WindowRect.width - 15),
                            GUILayout.Height(2));
                    ...
                    if (settings.AlarmsEnabled)
                    {
                        ...
                        if (settings.Resources[ResourceID].AlarmEnabled)
                        {
                            ...
                            switch (lstResources[ResourceID].MonitorState)
                            {
                                case ARPResource.MonitorStateEnum.Alert:
                                    if (lstResources[ResourceID].AlarmState !=
                                        ARPResource.AlarmStateEnum.Unacknowledged || DateTime.Now.Millisecond < 500)
                        ...
                        if (GUILayout.Button(contAlarm, Styles.styleAlarmButton))
                        {
                            if (lstResources[ResourceID].AlarmState == ARPResource.AlarmStateEnum.Unacknowledged)
                                ...
                                settings.Resources[ResourceID].AlarmEnabled =
                                    !settings.Resources[ResourceID].AlarmEnabled;
                        }
                    }

                    //Is this resource selected
                    var Highlight = SelectedResources.ContainsKey(ResourceID) &&
                                    SelectedResources[ResourceID].AllVisible;
                    ...
                    if (!settings.SplitLastStage || !lstResources[ResourceID].ResourceConfig.SplitLastStage ||
                        ...
                        (lstResources[ResourceID].ResourceDef.resourceFlowMode == ResourceFlowMode.ALL_VESSEL ||
                         lstResources[ResourceID].ResourceDef.resourceFlowMode ==
                    ...
                    )
                    {
                        ...
                        if (Drawing.DrawResourceBar(rectBar, lstResources[ResourceID], Styles.styleBarGreen_Back,
                                Styles.styleBarGreen, Styles.styleBarGreen_Thin, settings.ShowRates, Highlight,
                                Styles.styleBarHighlight))
                            //MonoBehaviourExtended.LogFormatted_DebugOnly("Clicked");
                            SelectedResources.TogglePartResourceVisible(ResourceID);
                    }
                    else
                    {
                        var FullVesselBarOffset = settings.StageBarOnRight ? Icon2BarOffset_Left : Icon2BarOffset_Right;
                        var StageBarOffset = settings.StageBarOnRight ? Icon2BarOffset_Right : Icon2BarOffset_Left;
                        ...
                        {
                            Highlight = SelectedResources.ContainsKey(ResourceID) &&
                                        SelectedResources[ResourceID].LastStageVisible;
                            rectBar = Drawing.CalcBarRect(rectIcon, StageBarOffset, 135, 15); //was 120
                            if (Drawing.DrawResourceBar(rectBar, lstResourcesLastStage[ResourceID],
                                Styles.styleBarBlue_Back, Styles.styleBarBlue, Styles.styleBarBlue_Thin,
                                settings.ShowRates, Highlight, Styles.styleBarHighlight))
            ...
            if (settings.StagingEnabled)
            {
                ...
                else
                {
                    if (GUILayout.Button("Auto:", Styles.styleStageTextHead, GUILayout.Width(50)))
                        settings.AutoStagingEnabled = !settings.AutoStagingEnabled;
                    GUILayout.Label(StageManager.CurrentStage.ToString(), Styles.styleStageTextHead,
                        GUILayout.Width(20));
                    GUILayout.Label("to", Styles.styleStageTextHead, GUILayout.Width(30));
                    GUILayout.Label(mbARP.AutoStagingTerminateAt.ToString(), Styles.styleStageTextHead,
                        GUILayout.Width(30));
                    DrawHorizontalSlider(ref mbARP.AutoStagingTerminateAt, 0, mbARP.AutoStagingMaxStage);
                    GUILayout.EndHorizontal();
                    GUILayout.BeginHorizontal();

                    var strButtonArm = "Arm";
            ...
            // ShowBase Button
            var blnShowBase = settings.ShowBase;
            if (DrawToggle(ref blnShowBase,
                new GUIContent(Resources.btnViewBaseActive,
                    blnShowBase
                        ? "Back to Vessel Display"
                        : "Show Base Display\r\nAll resource within 2km of active vessel"),
                SkinsLibrary.GetStyle(SkinsLibrary.CurrentSkin, "ButtonToggle").PaddingChange(1), GUILayout.Width(23)))
            ...
            // ShowTime Button
            var blnShowTimeRem = settings.ShowTimeRem;
            if (DrawToggle(ref blnShowTimeRem,
                new GUIContent(Resources.btnViewTimes, blnShowTimeRem ? "Hide time Remianing" : "Show Time Remaining"),
                SkinsLibrary.GetStyle(SkinsLibrary.CurrentSkin, "ButtonToggle").PaddingChange(1), GUILayout.Width(23)))
            ...
            // ShowAll Button
            var blnToggleHidden = KSPAlternateResourcePanel.ShowAll;
            if (DrawToggle(ref blnToggleHidden, new GUIContent(Resources.btnViewAll, "Toggle Hidden Resources"),
                SkinsLibrary.GetStyle(SkinsLibrary.CurrentSkin, "ButtonToggle").PaddingChange(1), GUILayout.Width(23)))
            ...
            {
                btnMinMax.image = Resources.btnChevronUp;
                btnMinMax.tooltip = "Hide Settings";
            }
            ...
            if (GUILayout.Button(btnMinMax, "ButtonSettings", GUILayout.Width(23)))
            {
                ...
                {
                    windowSettings.ddlSettingsTab.SelectedIndex = (int) ARPWindowSettings.SettingsTabs.About;
