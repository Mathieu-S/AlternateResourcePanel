...
namespace KSPAlternateResourcePanel
{
    internal class Drawing
    {
        ...
        internal static GUIContent GetResourceIcon(string ResourceName)
        {
            ...
            {
                contLabel = new GUIContent(Regex.Replace(ResourceName, "[^A-Z]", ""));
        ...
        internal static bool DrawFlowControlButton(Rect rectBar, bool FlowState)
        {
            var blnReturn = false;
        ...
        internal static bool DrawResourceBar(Rect rectBar, ARPResource Res, GUIStyle styleBack, GUIStyle styleFront,
            GUIStyle styleFrontThin, bool ShowRates, bool Highlight, GUIStyle HighlightStyle)
        {
            var blnReturn = false;
            var fltBarRemainRatio = (float) Res.Amount / (float) Res.MaxAmount;
            ...
            if (Highlight)
            {
                ...
                rectHighlight.x -= 2;
                rectHighlight.y -= 2;
                rectHighlight.width += 4;
                rectHighlight.height += 4;
            ...
            if (rectBar.width * fltBarRemainRatio > 1)
                DrawBarScaled(rectBar, Res, styleFront, styleFrontThin, fltBarRemainRatio);

            if (!KSPAlternateResourcePanel.settings.ShowTimeRem)
            {
                ////add amounts
                DrawUsage(rectBar, Res, ShowRates);
                ////add rate
                if (ShowRates) DrawRate(rectBar, Res);
            }
            else
            {
                DrawTimeRemaining(rectBar, Res);
            }

            return blnReturn;
        }

        internal static Rect CalcBarRect(Rect rectIcon, int Icon2BarOffset, int Width, int Height)
        {
        ...
        internal static void DrawBarScaled(Rect rectStart, ARPResource Res, GUIStyle Style, GUIStyle StyleNarrow,
            float Scale)
        {
        ...
        internal static void DrawUsage(Rect rectStart, ARPResource Res, bool ShowRates, bool IgnoreInstants = false)
        {
            ...
            var strUsageString = string.Format("{0} / {1}", Res.AmountFormatted, Res.MaxAmountFormatted);
            ;
        ...
        internal static void DrawTimeRemaining(Rect rectStart, ARPResource Res)
        {
            ...
            if (Res.Rate < 0)
            {
                ...
                var TimeRemaining = Math.Abs((Res.MaxAmount - Res.Amount) / Res.Rate);
                var strTime = FormatTime(TimeRemaining);
            ...
            else if (Res.Rate > 0)
            {
                ...
                var TimeRemaining = Math.Abs(Res.Amount / Res.Rate);
                var strTime = FormatTime(TimeRemaining);
                GUI.Label(rectTemp, strTime, Styles.styleBarText);
            }
            else
        ...
        internal static string FormatTime(double TimeInSecs)
        {
            ...
            var Second = TimeInSecs % 60;
            var Minute = Math.Truncate(TimeInSecs / 60 % 60);
            var HourRaw = TimeInSecs / 60 / 60;
